---
layout: post
title: 'Maven生成可以直接运行的jar包的多种方式'
tags: [code]
---

**转载自[叉叉哥的博客](http://xxgblog.com/2015/08/07/maven-create-executable-jar/)**
...
`<mainClass>com.xxg.Main</mainClass>`指定MANIFEST.MF中的Main-Class，`<addClasspath>true</addClasspath>`会在MANIFEST.MF加上Class-Path项并配置依赖包，`<classpathPrefix>lib/</classpathPrefix>`指定依赖包所在目录。
...
配置完成后，通过`mvn package`指令打包，会在target目录下生成jar包，并将依赖包拷贝到target/lib目录下
